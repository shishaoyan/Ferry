...
public class TraceDataUtils {
    ...
    public static void structuredDataToStack(long[] buffer, LinkedList<MethodItem> result, boolean isStrict, long endTime) {
            ...
            if (isIn(trueId)) {
                ...
                if (!rawData.isEmpty()) {
                    ...
                    while (getMethodId(in) != methodId && !rawData.isEmpty()) {
                        FerryLog.w(TAG, "[structuredDataToStack] method[%s] not match in! pop[%s] to continue find!", in, methodId);
                        in = rawData.pop();
                        depth--;
                    }
                    if (lastInId == methodId && methodId == MethodMonitor.METHOD_ID_DISPATCH) {
                        continue;
                    }
                    ...
                    if (during < 0) {
                        FerryLog.e(TAG, "[structuredDataToStack] trace during invalid:%d", during);
                        rawData.clear();
                        result.clear();
                        return;
                    }
                    ...
                    FerryLog.w(TAG, "[structuredDataToStack] method[%s] not found in! ", methodId);
                }
            }
        }
        ...
        while (!rawData.isEmpty() && isStrict) {
            ...
            long inTime = getTime(trueId) + MethodMonitor.getDiffTime();
            FerryLog.w(TAG, "[structuredDataToStack] has never out method[%s], isIn:%s, rawData size:%s", methodId, isIn, rawData.size());
            if (!isIn) {
